...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/validate_json \
  -H 'Content-Type: application/json' \
  -d '{"json":"{\"name\": \"John\", \"age\": 30}"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/validate_json`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
